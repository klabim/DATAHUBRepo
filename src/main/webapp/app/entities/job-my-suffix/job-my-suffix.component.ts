...
export class JobMySuffixComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.jobService
      ...
      .subscribe(
        (res: HttpResponse<IJobMySuffix[]>) => this.paginateJobs(res.body, res.headers),
  ...
  trackId(index: number, item: IJobMySuffix) {
    return item.id;
  }
  ...
  private paginateJobs(data: IJobMySuffix[], headers: HttpHeaders) {
